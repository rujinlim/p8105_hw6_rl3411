...
To build a model, let's use the `stepAIC()` function in the `MASS` package to carry out model building with a stepwise regression approach as learned in the P8130 class, following the guidelines [here](http://www.sthda.com/english/articles/37-model-selection-essentials-in-r/154-stepwise-regression-essentials-in-r/#:~:text=The%20stepwise%20regression%20(or%20stepwise,model%20that%20lowers%20prediction%20error.). 
